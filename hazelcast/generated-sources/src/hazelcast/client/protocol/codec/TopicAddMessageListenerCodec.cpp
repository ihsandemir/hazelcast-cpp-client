...
namespace hazelcast {
    namespace client {
        namespace protocol {
            namespace codec {
                const TopicMessageType TopicAddMessageListenerCodec::RequestParameters::TYPE = TOPIC_ADDMESSAGELISTENER;
                const bool TopicAddMessageListenerCodec::RequestParameters::RETRYABLE = true;
                ...
                void TopicAddMessageListenerCodec::AbstractEventHandler::handle(std::auto_ptr<protocol::ClientMessage> clientMessage) {
                    ...
                    switch (messageType) {
                        ...
                        default:
                            char buf[300];
                            sprintf(buf, "[TopicAddMessageListenerCodec::AbstractEventHandler::handle] Unknown message type (%d) received on event handler.", clientMessage->getMessageType());

...
namespace hazelcast {
    namespace client {
        namespace protocol {
            namespace codec {
                const ClientMessageType ClientAddPartitionLostListenerCodec::RequestParameters::TYPE = CLIENT_ADDPARTITIONLOSTLISTENER;
                const bool ClientAddPartitionLostListenerCodec::RequestParameters::RETRYABLE = true;
                ...
                void ClientAddPartitionLostListenerCodec::AbstractEventHandler::handle(std::auto_ptr<protocol::ClientMessage> clientMessage) {
                    ...
                    switch (messageType) {
                        ...
                        default:
                            char buf[300];
                            sprintf(buf, "[ClientAddPartitionLostListenerCodec::AbstractEventHandler::handle] Unknown message type (%d) received on event handler.", clientMessage->getMessageType());

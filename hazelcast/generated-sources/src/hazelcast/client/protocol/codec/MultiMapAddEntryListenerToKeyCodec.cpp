...
namespace hazelcast {
    namespace client {
        namespace protocol {
            namespace codec {
                const MultiMapMessageType MultiMapAddEntryListenerToKeyCodec::RequestParameters::TYPE = MULTIMAP_ADDENTRYLISTENERTOKEY;
                const bool MultiMapAddEntryListenerToKeyCodec::RequestParameters::RETRYABLE = true;
                ...
                void MultiMapAddEntryListenerToKeyCodec::AbstractEventHandler::handle(std::auto_ptr<protocol::ClientMessage> clientMessage) {
                    ...
                    switch (messageType) {
                        ...
                        default:
                            char buf[300];
                            sprintf(buf, "[MultiMapAddEntryListenerToKeyCodec::AbstractEventHandler::handle] Unknown message type (%d) received on event handler.", clientMessage->getMessageType());

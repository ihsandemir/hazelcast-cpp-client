...
namespace hazelcast {
    namespace client {
        namespace protocol {
            namespace codec {
                const MapMessageType MapAddEntryListenerWithPredicateCodec::RequestParameters::TYPE = MAP_ADDENTRYLISTENERWITHPREDICATE;
                const bool MapAddEntryListenerWithPredicateCodec::RequestParameters::RETRYABLE = true;
                ...
                void MapAddEntryListenerWithPredicateCodec::AbstractEventHandler::handle(std::auto_ptr<protocol::ClientMessage> clientMessage) {
                    ...
                    switch (messageType) {
                        ...
                        default:
                            char buf[300];
                            sprintf(buf, "[MapAddEntryListenerWithPredicateCodec::AbstractEventHandler::handle] Unknown message type (%d) received on event handler.", clientMessage->getMessageType());

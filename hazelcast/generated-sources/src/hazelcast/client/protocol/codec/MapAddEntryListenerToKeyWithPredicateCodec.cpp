...
namespace hazelcast {
    namespace client {
        namespace protocol {
            namespace codec {
                const MapMessageType MapAddEntryListenerToKeyWithPredicateCodec::RequestParameters::TYPE = MAP_ADDENTRYLISTENERTOKEYWITHPREDICATE;
                const bool MapAddEntryListenerToKeyWithPredicateCodec::RequestParameters::RETRYABLE = true;
                ...
                void MapAddEntryListenerToKeyWithPredicateCodec::AbstractEventHandler::handle(std::auto_ptr<protocol::ClientMessage> clientMessage) {
                    ...
                    switch (messageType) {
                        ...
                        default:
                            char buf[300];
                            sprintf(buf, "[MapAddEntryListenerToKeyWithPredicateCodec::AbstractEventHandler::handle] Unknown message type (%d) received on event handler.", clientMessage->getMessageType());

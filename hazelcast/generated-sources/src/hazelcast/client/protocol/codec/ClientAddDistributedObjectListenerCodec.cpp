...
namespace hazelcast {
    namespace client {
        namespace protocol {
            namespace codec {
                const ClientMessageType ClientAddDistributedObjectListenerCodec::RequestParameters::TYPE = CLIENT_ADDDISTRIBUTEDOBJECTLISTENER;
                const bool ClientAddDistributedObjectListenerCodec::RequestParameters::RETRYABLE = true;
                ...
                void ClientAddDistributedObjectListenerCodec::AbstractEventHandler::handle(std::auto_ptr<protocol::ClientMessage> clientMessage) {
                    ...
                    switch (messageType) {
                        ...
                        default:
                            char buf[300];
                            sprintf(buf, "[ClientAddDistributedObjectListenerCodec::AbstractEventHandler::handle] Unknown message type (%d) received on event handler.", clientMessage->getMessageType());

...
namespace hazelcast {
    namespace client {
        namespace protocol {
            namespace codec {
                const SetMessageType SetAddListenerCodec::RequestParameters::TYPE = SET_ADDLISTENER;
                const bool SetAddListenerCodec::RequestParameters::RETRYABLE = true;
                ...
                void SetAddListenerCodec::AbstractEventHandler::handle(std::auto_ptr<protocol::ClientMessage> clientMessage) {
                    ...
                    switch (messageType) {
                        ...
                        default:
                            char buf[300];
                            sprintf(buf, "[SetAddListenerCodec::AbstractEventHandler::handle] Unknown message type (%d) received on event handler.", clientMessage->getMessageType());

...
namespace hazelcast {
    namespace client {
        namespace protocol {
            namespace codec {
                const MapMessageType MapAddEntryListenerToKeyCodec::RequestParameters::TYPE = MAP_ADDENTRYLISTENERTOKEY;
                const bool MapAddEntryListenerToKeyCodec::RequestParameters::RETRYABLE = true;
                ...
                void MapAddEntryListenerToKeyCodec::AbstractEventHandler::handle(std::auto_ptr<protocol::ClientMessage> clientMessage) {
                    ...
                    switch (messageType) {
                        ...
                        default:
                            char buf[300];
                            sprintf(buf, "[MapAddEntryListenerToKeyCodec::AbstractEventHandler::handle] Unknown message type (%d) received on event handler.", clientMessage->getMessageType());

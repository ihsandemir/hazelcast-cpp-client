    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MapPutTransientParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)MapPutTransientParameters::TYPE);
                        clientMessage->set(name).set(key).set(value).set(threadId).set(ttl);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MapPutTransientParameters::calculateDataSize(
                            const std::string &name, 
                            const serialization::pimpl::Data &key, 
                            const serialization::pimpl::Data &value, 
                            const int64_t &threadId, 

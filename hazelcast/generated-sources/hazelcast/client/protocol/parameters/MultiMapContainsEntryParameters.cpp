    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MultiMapContainsEntryParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)MultiMapContainsEntryParameters::TYPE);
                        clientMessage->set(name).set(key).set(value).set(threadId);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MultiMapContainsEntryParameters::calculateDataSize(
                            const std::string &name, 
                            const serialization::pimpl::Data &key, 
                            const serialization::pimpl::Data &value, 
                            const int64_t &threadId) {

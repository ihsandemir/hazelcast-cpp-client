    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MapExecuteOnKeysParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)MapExecuteOnKeysParameters::TYPE);
                        clientMessage->set(name).set(entryProcessor).set(keys);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MapExecuteOnKeysParameters::calculateDataSize(
                            const std::string &name, 
                            const serialization::pimpl::Data &entryProcessor, 
                            const std::vector<serialization::pimpl::Data> &keys) {

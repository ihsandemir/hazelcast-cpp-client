    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> CountDownLatchAwaitParameters::encode(
                            const std::string &name, 
                            const int64_t &timeout) {
                        int32_t requiredDataSize = calculateDataSize(name, timeout);
                        std::auto_ptr<ClientMessage> clientMessage = ClientMessage::createForEncode(requiredDataSize);
                        clientMessage->setMessageType((uint16_t)CountDownLatchAwaitParameters::TYPE);
                        clientMessage->set(name).set(timeout);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t CountDownLatchAwaitParameters::calculateDataSize(
                            const std::string &name, 
                            const int64_t &timeout) {

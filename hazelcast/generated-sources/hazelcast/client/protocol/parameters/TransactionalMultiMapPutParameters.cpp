    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> TransactionalMultiMapPutParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)TransactionalMultiMapPutParameters::TYPE);
                        clientMessage->set(name).set(txnId).set(threadId).set(key).set(value);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t TransactionalMultiMapPutParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &txnId, 
                            const int64_t &threadId, 
                            const serialization::pimpl::Data &key, 

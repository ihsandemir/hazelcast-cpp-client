    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> SetCompareAndRemoveAllParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)SetCompareAndRemoveAllParameters::TYPE);
                        clientMessage->set(name).set(valueSet);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t SetCompareAndRemoveAllParameters::calculateDataSize(
                            const std::string &name, 
                            const std::vector<serialization::pimpl::Data> &valueSet) {

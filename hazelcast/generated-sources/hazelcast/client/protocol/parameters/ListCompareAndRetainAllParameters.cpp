    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> ListCompareAndRetainAllParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)ListCompareAndRetainAllParameters::TYPE);
                        clientMessage->set(name).set(valueSet);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t ListCompareAndRetainAllParameters::calculateDataSize(
                            const std::string &name, 
                            const std::vector<serialization::pimpl::Data> &valueSet) {

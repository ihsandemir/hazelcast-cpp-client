    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> ListAddAllWithIndexParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)ListAddAllWithIndexParameters::TYPE);
                        clientMessage->set(name).set(index).set(valueList);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t ListAddAllWithIndexParameters::calculateDataSize(
                            const std::string &name, 
                            const int32_t &index, 
                            const std::vector<serialization::pimpl::Data> &valueList) {

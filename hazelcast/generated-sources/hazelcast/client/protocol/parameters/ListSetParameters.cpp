    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> ListSetParameters::encode(
                            const std::string &name, 
                            const int32_t &index, 
                            const serialization::pimpl::Data &value) {
                        int32_t requiredDataSize = calculateDataSize(name, index, value);
                        std::auto_ptr<ClientMessage> clientMessage = ClientMessage::createForEncode(requiredDataSize);
                        clientMessage->setMessageType((uint16_t)ListSetParameters::TYPE);
                        clientMessage->set(name).set(index).set(value);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t ListSetParameters::calculateDataSize(
                            const std::string &name, 
                            const int32_t &index, 
                            const serialization::pimpl::Data &value) {

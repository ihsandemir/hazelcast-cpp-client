...
#include "hazelcast/client/protocol/parameters/MapReduceForMultiMapParameters.h"
    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MapReduceForMultiMapParameters::encode(
                            ...
                            const std::string &multiMapName, 
                            const int32_t &chunkSize, 
                            const std::vector<serialization::pimpl::Data> &keys, 
                            const std::string &topologyChangedStrategy) {
                        int32_t requiredDataSize = calculateDataSize(name, jobId, predicate, mapper, combinerFactory, reducerFactory, multiMapName, chunkSize, keys, topologyChangedStrategy);
                        std::auto_ptr<ClientMessage> clientMessage = ClientMessage::createForEncode(requiredDataSize);
                        clientMessage->setMessageType((uint16_t)MapReduceForMultiMapParameters::TYPE);
                        clientMessage->set(name).set(jobId).set(predicate).set(mapper).set(combinerFactory).set(reducerFactory).set(multiMapName).set(chunkSize).set(keys).set(topologyChangedStrategy);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MapReduceForMultiMapParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &jobId, 
                            const serialization::pimpl::Data &predicate, 
                            const serialization::pimpl::Data &mapper, 
                        ...
                        return ClientMessage::HEADER_SIZE
                             ...
                             + ClientMessage::calculateDataSize(multiMapName)

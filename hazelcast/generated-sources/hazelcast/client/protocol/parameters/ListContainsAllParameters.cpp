    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> ListContainsAllParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)ListContainsAllParameters::TYPE);
                        clientMessage->set(name).set(valueSet);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t ListContainsAllParameters::calculateDataSize(
                            const std::string &name, 
                            const std::vector<serialization::pimpl::Data> &valueSet) {

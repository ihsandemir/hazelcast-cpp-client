    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> SetAddAllParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)SetAddAllParameters::TYPE);
                        clientMessage->set(name).set(valueList);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t SetAddAllParameters::calculateDataSize(
                            const std::string &name, 
                            const std::vector<serialization::pimpl::Data> &valueList) {

    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> TransactionalListRemoveParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)TransactionalListRemoveParameters::TYPE);
                        clientMessage->set(name).set(txnId).set(threadId).set(item);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t TransactionalListRemoveParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &txnId, 
                            const int64_t &threadId, 
                            const serialization::pimpl::Data &item) {

    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> TransactionalMapDeleteParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)TransactionalMapDeleteParameters::TYPE);
                        clientMessage->set(name).set(txnId).set(threadId).set(key);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t TransactionalMapDeleteParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &txnId, 
                            const int64_t &threadId, 
                            const serialization::pimpl::Data &key) {

...
#include "hazelcast/client/protocol/parameters/MapReduceForCustomParameters.h"
    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MapReduceForCustomParameters::encode(
                        ...
                        int32_t requiredDataSize = calculateDataSize(name, jobId, predicate, mapper, combinerFactory, reducerFactory, keyValueSource, chunkSize, keys, topologyChangedStrategy);
                        std::auto_ptr<ClientMessage> clientMessage = ClientMessage::createForEncode(requiredDataSize);
                        clientMessage->setMessageType((uint16_t)MapReduceForCustomParameters::TYPE);
                        clientMessage->set(name).set(jobId).set(predicate).set(mapper).set(combinerFactory).set(reducerFactory).set(keyValueSource).set(chunkSize).set(keys).set(topologyChangedStrategy);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MapReduceForCustomParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &jobId, 
                            const serialization::pimpl::Data &predicate, 
                            const serialization::pimpl::Data &mapper, 
                        ...
                        return ClientMessage::HEADER_SIZE
                             ...
                             + ClientMessage::calculateDataSize(keyValueSource)

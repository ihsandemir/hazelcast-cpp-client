    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> TransactionalMapReplaceIfSameParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)TransactionalMapReplaceIfSameParameters::TYPE);
                        clientMessage->set(name).set(txnId).set(threadId).set(key).set(oldValue).set(newValue);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t TransactionalMapReplaceIfSameParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &txnId, 
                            const int64_t &threadId, 
                            const serialization::pimpl::Data &key, 

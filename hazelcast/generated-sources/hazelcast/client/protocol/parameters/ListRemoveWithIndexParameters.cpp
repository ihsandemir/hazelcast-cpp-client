    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> ListRemoveWithIndexParameters::encode(
                            const std::string &name, 
                            const int32_t &index) {
                        int32_t requiredDataSize = calculateDataSize(name, index);
                        std::auto_ptr<ClientMessage> clientMessage = ClientMessage::createForEncode(requiredDataSize);
                        clientMessage->setMessageType((uint16_t)ListRemoveWithIndexParameters::TYPE);
                        clientMessage->set(name).set(index);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t ListRemoveWithIndexParameters::calculateDataSize(
                            const std::string &name, 
                            const int32_t &index) {

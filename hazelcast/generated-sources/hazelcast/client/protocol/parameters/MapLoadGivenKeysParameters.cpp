    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MapLoadGivenKeysParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)MapLoadGivenKeysParameters::TYPE);
                        clientMessage->set(name).set(keys).set(replaceExistingValues);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MapLoadGivenKeysParameters::calculateDataSize(
                            const std::string &name, 
                            const std::vector<serialization::pimpl::Data> &keys, 
                            const bool &replaceExistingValues) {

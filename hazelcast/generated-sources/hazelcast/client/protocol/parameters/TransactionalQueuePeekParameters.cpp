    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> TransactionalQueuePeekParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)TransactionalQueuePeekParameters::TYPE);
                        clientMessage->set(name).set(txnId).set(threadId).set(timeout);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t TransactionalQueuePeekParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &txnId, 
                            const int64_t &threadId, 
                            const int64_t &timeout) {

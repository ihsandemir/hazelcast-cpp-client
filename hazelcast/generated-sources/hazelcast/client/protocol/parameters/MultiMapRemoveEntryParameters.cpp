    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MultiMapRemoveEntryParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)MultiMapRemoveEntryParameters::TYPE);
                        clientMessage->set(name).set(key).set(value).set(threadId);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MultiMapRemoveEntryParameters::calculateDataSize(
                            const std::string &name, 
                            const serialization::pimpl::Data &key, 
                            const serialization::pimpl::Data &value, 
                            const int64_t &threadId) {

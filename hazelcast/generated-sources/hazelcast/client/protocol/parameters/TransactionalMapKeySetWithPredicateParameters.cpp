    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> TransactionalMapKeySetWithPredicateParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)TransactionalMapKeySetWithPredicateParameters::TYPE);
                        clientMessage->set(name).set(txnId).set(threadId).set(predicate);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t TransactionalMapKeySetWithPredicateParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &txnId, 
                            const int64_t &threadId, 
                            const serialization::pimpl::Data &predicate) {

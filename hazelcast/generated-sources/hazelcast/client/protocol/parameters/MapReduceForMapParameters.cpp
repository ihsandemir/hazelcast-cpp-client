...
#include "hazelcast/client/protocol/parameters/MapReduceForMapParameters.h"
    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MapReduceForMapParameters::encode(
                        ...
                        int32_t requiredDataSize = calculateDataSize(name, jobId, predicate, mapper, combinerFactory, reducerFactory, mapName, chunkSize, keys, topologyChangedStrategy);
                        std::auto_ptr<ClientMessage> clientMessage = ClientMessage::createForEncode(requiredDataSize);
                        clientMessage->setMessageType((uint16_t)MapReduceForMapParameters::TYPE);
                        clientMessage->set(name).set(jobId).set(predicate).set(mapper).set(combinerFactory).set(reducerFactory).set(mapName).set(chunkSize).set(keys).set(topologyChangedStrategy);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MapReduceForMapParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &jobId, 
                            const serialization::pimpl::Data &predicate, 
                            const serialization::pimpl::Data &mapper, 
                        ...
                        return ClientMessage::HEADER_SIZE
                             ...
                             + ClientMessage::calculateDataSize(mapName)

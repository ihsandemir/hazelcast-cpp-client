    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MapGetAsyncParameters::encode(
                            const std::string &name, 
                            const serialization::pimpl::Data &key, 
                            const int64_t &threadId) {
                        int32_t requiredDataSize = calculateDataSize(name, key, threadId);
                        std::auto_ptr<ClientMessage> clientMessage = ClientMessage::createForEncode(requiredDataSize);
                        clientMessage->setMessageType((uint16_t)MapGetAsyncParameters::TYPE);
                        clientMessage->set(name).set(key).set(threadId);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MapGetAsyncParameters::calculateDataSize(
                            const std::string &name, 
                            const serialization::pimpl::Data &key, 
                            const int64_t &threadId) {

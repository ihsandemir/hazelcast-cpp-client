    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MapExecuteWithPredicateParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)MapExecuteWithPredicateParameters::TYPE);
                        clientMessage->set(name).set(entryProcessor).set(predicate);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MapExecuteWithPredicateParameters::calculateDataSize(
                            const std::string &name, 
                            const serialization::pimpl::Data &entryProcessor, 
                            const serialization::pimpl::Data &predicate) {

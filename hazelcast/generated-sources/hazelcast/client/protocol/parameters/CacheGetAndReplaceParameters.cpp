...
#include "hazelcast/client/protocol/parameters/CacheGetAndReplaceParameters.h"
    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> CacheGetAndReplaceParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)CacheGetAndReplaceParameters::TYPE);
                        clientMessage->set(name).set(key).set(value).set(expiryPolicy);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t CacheGetAndReplaceParameters::calculateDataSize(
                            const std::string &name, 
                            const serialization::pimpl::Data &key, 
                            const serialization::pimpl::Data &value, 
                            const serialization::pimpl::Data &expiryPolicy) {

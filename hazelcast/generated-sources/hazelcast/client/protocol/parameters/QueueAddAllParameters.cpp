    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> QueueAddAllParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)QueueAddAllParameters::TYPE);
                        clientMessage->set(name).set(dataList);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t QueueAddAllParameters::calculateDataSize(
                            const std::string &name, 
                            const std::vector<serialization::pimpl::Data> &dataList) {

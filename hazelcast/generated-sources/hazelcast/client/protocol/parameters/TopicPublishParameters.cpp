    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> TopicPublishParameters::encode(
                            const std::string &name, 
                            const serialization::pimpl::Data &message) {
                        int32_t requiredDataSize = calculateDataSize(name, message);
                        std::auto_ptr<ClientMessage> clientMessage = ClientMessage::createForEncode(requiredDataSize);
                        clientMessage->setMessageType((uint16_t)TopicPublishParameters::TYPE);
                        clientMessage->set(name).set(message);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t TopicPublishParameters::calculateDataSize(
                            const std::string &name, 
                            const serialization::pimpl::Data &message) {
                        return ClientMessage::HEADER_SIZE
                             + ClientMessage::calculateDataSize(name)
                             + ClientMessage::calculateDataSize(message);
                    }
            }
        }
    }

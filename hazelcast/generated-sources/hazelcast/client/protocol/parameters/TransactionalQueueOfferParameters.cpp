    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> TransactionalQueueOfferParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)TransactionalQueueOfferParameters::TYPE);
                        clientMessage->set(name).set(txnId).set(threadId).set(item).set(timeout);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t TransactionalQueueOfferParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &txnId, 
                            const int64_t &threadId, 
                            const serialization::pimpl::Data &item, 

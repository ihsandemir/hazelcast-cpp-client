    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> TransactionalSetAddParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)TransactionalSetAddParameters::TYPE);
                        clientMessage->set(name).set(txnId).set(threadId).set(item);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t TransactionalSetAddParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &txnId, 
                            const int64_t &threadId, 
                            const serialization::pimpl::Data &item) {

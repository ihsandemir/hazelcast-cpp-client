    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> ListCompareAndRemoveAllParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)ListCompareAndRemoveAllParameters::TYPE);
                        clientMessage->set(name).set(valueSet);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t ListCompareAndRemoveAllParameters::calculateDataSize(
                            const std::string &name, 
                            const std::vector<serialization::pimpl::Data> &valueSet) {

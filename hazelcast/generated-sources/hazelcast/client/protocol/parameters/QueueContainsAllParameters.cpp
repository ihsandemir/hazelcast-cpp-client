    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> QueueContainsAllParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)QueueContainsAllParameters::TYPE);
                        clientMessage->set(name).set(dataList);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t QueueContainsAllParameters::calculateDataSize(
                            const std::string &name, 
                            const std::vector<serialization::pimpl::Data> &dataList) {

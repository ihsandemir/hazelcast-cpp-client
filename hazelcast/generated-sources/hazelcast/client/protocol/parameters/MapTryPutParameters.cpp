    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MapTryPutParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)MapTryPutParameters::TYPE);
                        clientMessage->set(name).set(key).set(value).set(threadId).set(timeout);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MapTryPutParameters::calculateDataSize(
                            const std::string &name, 
                            const serialization::pimpl::Data &key, 
                            const serialization::pimpl::Data &value, 
                            const int64_t &threadId, 

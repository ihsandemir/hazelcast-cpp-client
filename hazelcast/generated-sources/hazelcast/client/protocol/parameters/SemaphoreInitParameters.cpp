    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> SemaphoreInitParameters::encode(
                            const std::string &name, 
                            const int32_t &permits) {
                        int32_t requiredDataSize = calculateDataSize(name, permits);
                        std::auto_ptr<ClientMessage> clientMessage = ClientMessage::createForEncode(requiredDataSize);
                        clientMessage->setMessageType((uint16_t)SemaphoreInitParameters::TYPE);
                        clientMessage->set(name).set(permits);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t SemaphoreInitParameters::calculateDataSize(
                            const std::string &name, 
                            const int32_t &permits) {

    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MapTryLockParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)MapTryLockParameters::TYPE);
                        clientMessage->set(name).set(key).set(threadId).set(timeout);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MapTryLockParameters::calculateDataSize(
                            const std::string &name, 
                            const serialization::pimpl::Data &key, 
                            const int64_t &threadId, 
                            const int64_t &timeout) {

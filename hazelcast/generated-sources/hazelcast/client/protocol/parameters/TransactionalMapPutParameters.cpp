    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> TransactionalMapPutParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)TransactionalMapPutParameters::TYPE);
                        clientMessage->set(name).set(txnId).set(threadId).set(key).set(value).set(ttl);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t TransactionalMapPutParameters::calculateDataSize(
                            const std::string &name, 
                            const std::string &txnId, 
                            const int64_t &threadId, 
                            const serialization::pimpl::Data &key, 

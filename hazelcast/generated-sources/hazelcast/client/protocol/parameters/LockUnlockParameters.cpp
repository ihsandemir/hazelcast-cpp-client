    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> LockUnlockParameters::encode(
                            const std::string &name, 
                            const int64_t &threadId) {
                        int32_t requiredDataSize = calculateDataSize(name, threadId);
                        std::auto_ptr<ClientMessage> clientMessage = ClientMessage::createForEncode(requiredDataSize);
                        clientMessage->setMessageType((uint16_t)LockUnlockParameters::TYPE);
                        clientMessage->set(name).set(threadId);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t LockUnlockParameters::calculateDataSize(
                            const std::string &name, 
                            const int64_t &threadId) {

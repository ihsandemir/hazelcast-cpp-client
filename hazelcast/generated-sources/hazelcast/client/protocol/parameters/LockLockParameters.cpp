    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> LockLockParameters::encode(
                            const std::string &name, 
                            const int64_t &leaseTime, 
                            const int64_t &threadId) {
                        int32_t requiredDataSize = calculateDataSize(name, leaseTime, threadId);
                        std::auto_ptr<ClientMessage> clientMessage = ClientMessage::createForEncode(requiredDataSize);
                        clientMessage->setMessageType((uint16_t)LockLockParameters::TYPE);
                        clientMessage->set(name).set(leaseTime).set(threadId);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t LockLockParameters::calculateDataSize(
                            const std::string &name, 
                            const int64_t &leaseTime, 
                            const int64_t &threadId) {
                        return ClientMessage::HEADER_SIZE
                             + ClientMessage::calculateDataSize(name)
                             + ClientMessage::calculateDataSize(leaseTime)

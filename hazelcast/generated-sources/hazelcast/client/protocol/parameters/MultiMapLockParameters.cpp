    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MultiMapLockParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)MultiMapLockParameters::TYPE);
                        clientMessage->set(name).set(key).set(threadId).set(ttl);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MultiMapLockParameters::calculateDataSize(
                            const std::string &name, 
                            const serialization::pimpl::Data &key, 
                            const int64_t &threadId, 
                            const int64_t &ttl) {

    ...
    namespace client {
        namespace protocol {
            namespace parameters {
                    std::auto_ptr<ClientMessage> MapLoadAllParameters::encode(
                        ...
                        clientMessage->setMessageType((uint16_t)MapLoadAllParameters::TYPE);
                        clientMessage->set(name).set(replaceExistingValues);
                        clientMessage->updateFrameLength();
                        return clientMessage;
                    }

                    int32_t MapLoadAllParameters::calculateDataSize(
                            const std::string &name, 
                            const bool &replaceExistingValues) {
